 ...
 * Version 1.4
 * Mods by youtube channel: sasiskas
 */
#include <avr/io.h>
...
const uint16_t temp_ambC    = 28;                                           // Average ambient temperature
...
class HOTGUN_CFG : public CONFIG {
    ...
    private:
        ...
        const   uint16_t ambient_temp = 50;
        const   uint16_t ambient_tempC= 28;
};
...
void BUZZER::lowBeep(void) {   
  pinMode(buzzer_pin, OUTPUT);                  //Heat Gun Is Cold
        digitalWrite(buzzer_pin, HIGH);
        delay(160);
        digitalWrite(buzzer_pin, LOW);
}
...
class DSPL : protected LiquidCrystal_I2C {
    ...
    private:
		...
		char 	temp_units;
        const   uint8_t custom_symbols[6][8] = {
                          { 0b00110,                                        // Degree
                            0b01001,
                            0b01001,
                            0b00110,
...
void DSPL::tSet(uint16_t t, bool Celsius) {
    ...
    sprintf(buff, "%c%3d%c%c", (char)4, t, (char)1, temp_units);
...
void DSPL::tCurr(uint16_t t, bool Celsius) {
    ...
    if (t < 1000) {
        sprintf(buff, "%c%3d%c%c ", (char)5, t, (char)1, temp_units);
    } else {
...
void DSPL::fanSpeed(uint8_t s) {
    char buff[6];
    s = map(s, 0, 255, 0, 99);
    sprintf(buff, " %c%2d%c", (char)6, s, '%');
    LiquidCrystal_I2C::setCursor(6, 0);
    LiquidCrystal_I2C::print(buff);
}

void DSPL::fanSpeedCurr(uint8_t s) {                            //test
    char buff[6];												//test
    s = map(s, 0, 255, 0, 99);									//test
    sprintf(buff, " %c%2d%c", (char)2, s, '%');					//test
    LiquidCrystal_I2C::setCursor(6, 1);							//test
    LiquidCrystal_I2C::print(buff);								//test
}
...
class HOTGUN : public PID {
    ...
    private:
        ...
        const       uint8_t     max_power       = 70;
        const       uint16_t    min_fan_speed   = 99;
        const       uint16_t    max_fan_speed   = 255;
        const       uint16_t    max_cool_fan    = 255;
        const       uint16_t    temp_gun_cold   = 90;
            ...
            if (hg_fan.fanSpeed() < min_fan_speed) {
                ...
                if (isGunConnected()) {
                    if (isCold()) {                                         // FAN && connected && cold
						...
						uint16_t fan = max_cool_fan; //map(temp, temp_gun_cold, temp_set, max_cool_fan, min_fan_speed);
